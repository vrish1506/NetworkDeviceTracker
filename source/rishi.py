...
def makeLineToDict(_myDict_):
    ...
    fields = { "0": "Filesystem", "1": "1K-blocks", "2": "Used", "3": "Available", "4":"Use%", "5":"Mounted on"}
...
def AskForFilteredOutput(_myDict_):
    ...
    # For FileSytem _input_ get the line which has the information.

    elts_dict = _myDict_[_input_]
    
    # In the line, the fields are separated by ",", hence we need to split it. Then print the 4th field.
    print ("Rishi", elts_dict.split(",")[4])

    # print (_myDict_[_input_][_input1_]

    print("Filesystem %s " % elts_dict + '\n')
    
    retval = makeLineToDict(_myDict_)
    _input1_ = input("Enter the field to know about ?")
    print ("the information you asked for is ", retval[machine][_input_][_input1_])
...
#    nes_ = (line.strip() for line in _logfile_.readlines())
